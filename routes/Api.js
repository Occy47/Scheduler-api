...
router.get("/status", function (req, res, next) {
  ...
  var jsonRunning = '{"status": false}';
  var jsonOffline = '{"status": true}';
  var objRunning = JSON.parse(jsonRunning);
  var objeOffline = JSON.parse(jsonOffline);
  if (upTime > 0) {
    res.send(objRunning);
  } else {
    res.send(objeOffline);
  }
  //res.send("status: server is running for " + upTime + " seconds.");

  //res.sendStatus(200, "server running");
